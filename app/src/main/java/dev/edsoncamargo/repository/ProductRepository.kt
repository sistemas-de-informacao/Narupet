...
interface ProductRepository {
    ...
    fun listByName(@Path("name") name: String): Call<List<Product>>

    @GET(value = "/android/rest/produto/categoria/{id}")
    fun listByCategory(@Path("id") name: Int): Call<List<Product>>

    @GET(value = "/android/rest/produto/{name}/{category}")
    fun listByNameAndCategory(@Path("name") name: String, @Path("category") category: Int): Call<List<Product>>
}